...
struct emp *deletenode(struct emp *start,char str[])
{
  ...
  {
    if (strcmp(currentnode->ptr->fname,str) == 0)
    {
      if (prevnode == NULL)
      {
        start = start->next;
...
int main()
{
  ...
  display(head);
  char dname[15];
  printf("\n\nEnter the first name of the employee to be deleted: ");
  scanf("%s",dname);
  printf("\nLinkedlist after deleting the data of %s: \n\n",dname);
  head = deletenode(head,dname);
  display(head);
}
